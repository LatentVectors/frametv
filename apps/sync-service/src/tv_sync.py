...
def initiate_connection(ip_address: str, port: int = 8002) -> Tuple[bool, bool, str]:
    ...
    """
    # In mock mode, reset the PIN state when no token exists so tests consistently
    # require PIN entry on first connection attempts.
    if os.getenv("MOCK_TV", "").lower() == "true":
        token_file = get_token_file_path()
        if not os.path.exists(token_file):
            try:
                from tv_mock import reset_mock_state  # type: ignore
...
def authorize_with_pin(ip_address: str, port: int, pin: str) -> Tuple[bool, bool, str]:
    ...
    try:
        ...
        if os.getenv("MOCK_TV", "").lower() == "true":
            try:
                from tv_mock import start_authorization_flow  # type: ignore
        ...
        try:
            ...
            if token_saved:
                ...
                return (
                    True,
                    False,
                    f"Connected to {device_name} but token not saved. PIN may need to be entered manually.",
                )
            ...
            if (
                ...
                return (
                    False,
                    False,
                    f"PIN authorization required. The library may require interactive PIN entry. Error: {str(e)}",
                )
            else:
